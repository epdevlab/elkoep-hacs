...
async def test_async_get_options_flow(
    hass: HomeAssistant, default_config, altered_config
) -> None:
    """Test that options flow successfully updates configuration options."""
    config_entry = MockConfigEntry(domain=inels.DOMAIN, data=default_config)
    ...
    assert flow["step_id"] == "setup"

    # Simulate configuring the options
    result = await hass.config_entries.options.async_configure(
        flow["flow_id"], user_input=altered_config
    )
...
async def test_options_flow_various_errors(
    hass: HomeAssistant,
    mock_try_connection,
    error_code,
    expected_error,
    ...
    """Test options flow handles various errors correctly."""
    mock_try_connection.return_value = error_code
